...
import { UpdateChantierInput } from './update-chantier.input';

/**
 * Mettre a jour un chantier existant.
 */
export class UpdateChantierUseCase {
  constructor(private readonly repository: ChantierRepository) {}

  async execute(input: UpdateChantierInput): Promise<Result<Chantier>> {
    try {
      const alreadyExists = await this.repository.existsByName(input.name, input.id);
      if (alreadyExists) {
        throw new ChantierNameAlreadyExistsError();
      }

      const chantier = new Chantier(input.id, input.name);
      await this.repository.update(chantier);
      ...
      const message = error instanceof Error ? error.message : 'Error updating chantier';
